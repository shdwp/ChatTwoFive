...
internal sealed class IpcManager : IDisposable {
    ...
    public IpcManager(DalamudPluginInterface pluginInterface) {
        ...
        this.RegisterGate = this.Interface.GetIpcProvider<string>("ChatTwoFive.Register");
        this.RegisterGate.RegisterFunc(this.Register);

        this.AvailableGate = this.Interface.GetIpcProvider<object?>("ChatTwoFive.Available");

        this.UnregisterGate = this.Interface.GetIpcProvider<string, object?>("ChatTwoFive.Unregister");
        this.UnregisterGate.RegisterAction(this.Unregister);

        this.InvokeGate = this.Interface.GetIpcProvider<string, PlayerPayload?, ulong, Payload?, SeString?, SeString?, object?>("ChatTwoFive.Invoke");
